...
def Merge(data, left, middle, right, drawDataArray, sortSpeedTime):
    ...
    drawDataArray(data, ["#BCCEF8" if x >= left and x <= right else "#FAF7F0" for x in range(len(data))])
    time.sleep(sortSpeedTime)
...
def getColorArray(leght, left, middle, right):
    ...
    for i in range(leght):
        if i >= left and i <= right:
            ...
            colorArray.append("#FAF7F0") # Data values that we aren't currently working with are highlighted in red
